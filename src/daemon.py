        ...
        while True:
            try:
                ...
                if idle_duration >= 600.0:
                    daemon_log.info(f"Daemon has been idle for {idle_duration/60:.1f} minutes. Shutting down due to TTL.")
                    break
                continue
...
class DaemonClient:
    ...
    def run(self, tool_name: str, argv: List[str]):
        ...
        if not connected:
            ...
            daemon_log.info("Build daemon is not running. Starting background daemon.")
            ...
            if not connected:
                ...
                daemon_log.error("Could not connect to or start build daemon.")
                logger.done()
                sys.exit(1)
            else:
                logger.done()
        ...
        try:
            ...
            daemon_log.warning("Build cancelled by user.")
            logger.done()
        except OSError as e:
            from shell import Logger
            logger = Logger(enabled=True)
            ...
            daemon_log.exception("Error communicating with daemon")
            logger.done()
            sys.exit(1)
