...
class DaemonClient:
    ...
    def run(self, tool_name: str, argv: List[str]):
        ...
        if not connected:
            # Start daemon in the background and retry connection
            print("Build daemon is not running. Starting background daemon...")
            self.socket_path.parent.mkdir(parents=True, exist_ok=True)
            ...
            if not connected:
                print(
                    "Warning: Could not connect to daemon. Falling back to local execution...",
                    file=sys.stderr,
                )
                self._run_locally(tool_name, argv)
                return
        ...
        try:
            with daemon_connection(self.socket_path) as client_socket:
                client_socket.sendall(json.dumps(req).encode("utf-8"))
    ...
    if args.start_daemon:
        ...
        try:
            with daemon_connection(SOCKET_PATH) as client_socket:
                client_socket.sendall(json.dumps({"command": "stop"}).encode("utf-8"))
        ...
        try:
            with daemon_connection(SOCKET_PATH) as client_socket:
                client_socket.sendall(json.dumps({"command": "status"}).encode("utf-8"))
                print(client_socket.recv(1024).decode("utf-8"))
        except (ConnectionRefusedError, FileNotFoundError, OSError):
            print("Daemon is not running.")
    else:
