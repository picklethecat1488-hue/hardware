...
class CapacitiveSensingGenerator:
    ...
    def _generate_hatched_ground(self, width_mm: float, length_mm: float) -> List[HatchLine]:
        ...
        for i in range(-num_lines, num_lines):
            ...
            if x_min <= x_at_ymin <= x_max and (x_at_ymin, y_min) not in pts:
                pts.append((x_at_ymin, y_min))
            # Intersect with y = y_max
            x_at_ymax = y_max - offset
            if x_min <= x_at_ymax <= x_max and (x_at_ymax, y_max) not in pts:
                pts.append((x_at_ymax, y_max))

            if len(pts) == 2:
                lines.append(HatchLine(start=pts[0], end=pts[1], width_mm=line_w))
