...
def _wavefront_step_2layer(
    ...
    c_via = jnp.where(other < REACHABLE_THRESHOLD, other + via_penalty, INF_COST)
...
def _run_wavefront_chunk(
    ...
    def body_fn(_idx: int, c: jnp.ndarray) -> jnp.ndarray:
        return _wavefront_step_2layer(c, mask, grid_step, via_penalty)
...
class JaxPCBRouter:
    ...
    def route_net(
        ...
        # Prepare active obstacle mask for this net (temporarily unmasking start and target cells)
        ...
        for _ in range(max_chunks):
            cost = _run_wavefront_chunk(cost, mask_jax, self.grid_step, self.via_penalty, chunk_size)
        ...
        # Convert discrete grid path into physical segments and via models
        raw_pts = [self.grid_to_world(gl, gy, gx) for gl, gy, gx in grid_path]
        if not raw_pts:
            return [], []

        # Ensure exact start and end coordinate adherence
        raw_pts[0] = (start_pt[0], start_pt[1], start_layer)
        raw_pts[-1] = (end_pt[0], end_pt[1], end_layer)
        ...
        # Register placed traces and vias into obstacle tensor
        for tr in traces:
        ...
        for v in vias:
            v_r = v.pad_diameter_mm / 2.0 + 0.12
            self.add_obstacle(
                Obstacle(
    ...
    def route_batch(
        ...
        for _ in range(max_chunks):
            costs_jax = _wavefront_step_batch_2layer(costs_jax, masks_jax, self.grid_step, self.via_penalty, chunk_size)
        ...
        for b_idx, (s_pt, s_lay, e_pt, e_lay, net_name, width_mm) in enumerate(net_specs):
            ...
            while curr != (l0, y0, x0):
                ...
                if c_via_cand + self.via_penalty <= costs_final[b_idx, cl, cy, cx] + 1e-4 and c_via_cand < best_cost:
                    best_cost = c_via_cand
                    best_next = (other_l, cy, cx)
