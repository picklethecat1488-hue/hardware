...
from provider import Action, Mode, SUBASSEMBLIES, MODES
...
class TestTargetParser:
    ...
    def mock_router(self):
        ...
        router.manifest = {
            "tube/driver": {
                Action.PART: {MODES: [Mode.DEFAULT, Mode.PRINT], SUBASSEMBLIES: ["left", "right"]},
                Action.VIEW: {MODES: [Mode.DEFAULT]},
            },
            "tube/wire": {
                Action.PART: {MODES: [Mode.DEFAULT]},
            },
            "electronics/case": {
                Action.PART: {MODES: [Mode.DEFAULT, Mode.PRINT]},
            },
        }
    ...
    def test_parse_simple(self, parser):
        ...
        info = parser.parse("tube/driver", Action.PART)
    ...
    def test_parse_complex(self, parser):
        ...
        info = parser.parse("tube/driver_left:part/print", Action.PART)
        assert info.target == "tube/driver"
        assert info.subassembly == "left"
        assert info.action == Action.PART
    ...
    def test_get_names_combinations(self, parser):
        ...
        names_part = parser.get_names([Action.PART])
