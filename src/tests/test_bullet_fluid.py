...
class TestBulletFluid:
    ...
    def test_fluid_terminal_velocity(self, mode: str) -> None:
        ...
        try:
            ...
            bowl_col = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.05, 0.05, 0.01], physicsClientId=physics_client)
            tube_col = p.createCollisionShape(p.GEOM_CYLINDER, radius=0.002, height=0.05, physicsClientId=physics_client)
            vane_col = p.createCollisionShape(p.GEOM_CYLINDER, radius=0.002, height=0.01, physicsClientId=physics_client)
            body_id = p.createMultiBody(
                baseMass=0.0,
                baseCollisionShapeIndex=bowl_col,
                basePosition=[0.0, 0.0, 0.0],
            ...
            if fluid.spawner:
                for w_id in fluid.spawner.particle_body_ids:
                    p.setCollisionFilterPair(body_steel, w_id, -1, -1, enableCollision=0, physicsClientId=physics_client)
