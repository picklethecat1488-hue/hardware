...
class BoundaryConfig(BaseModel):
    ...
    def compute_surface_bounds(
        ...
        r_outer = r
        tray_z_min = -thick - 0.005
        tray_z_max = h + 0.060
        suction_z_min = h - ceil_thick - 0.002
        suction_z_max = h + 2.0 * ceil_thick
        spout_z_min = h - 0.0025
        drain_target_z = -thick - 0.005
        drain_influence_radius = max(0.030, dr_r * 1.5)
        wall_band_r_max = r + 0.015
